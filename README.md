...
- Загрузка по дате теперь менее актуальна, если удаленные файлы не будут догружаться, если их пост есть в списке url в !ids.txt
- Вынесены отдельные переменные. Вторая используется совместно с первой дл r34.
``` python
filename = '!ids.txt'
